...
func Test_Checker_Run(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := &checker{
				bus:        mockBus,
				cfg:        &config.Config{Updater: tt.updaterEnabled},
				log:        mockLog,
			...
			entry, cached := readCache(path)

			switch {
			case tt.expectedCacheTag != "":
				assert.True(t, cached, "expected cache to be present")
				assert.Equal(t, tt.expectedCacheTag, entry.Tag, "cache tag mismatch")
			default:
				assert.False(t, cached, "expected no cache to be present")
			}
		})
	}
}
