...
func Test_LogFilter_ToggleAll(t *testing.T) {
	tests := []struct {
		...
		{name: "Toggle all from disabled to enabled", setup: func(f LogFilter) {}, services: []string{"api", "web", "db"}, expected: map[string]bool{"api": true, "web": true, "db": true}},
		{name: "Toggle all from enabled to disabled", setup: func(f LogFilter) {
			for _, s := range []string{"api", "web", "db"} {
				f.Set(s, true)
			}
		}, services: []string{"api", "web", "db"}, expected: map[string]bool{"api": false, "web": false, "db": false}},
		{name: "Toggle with some enabled to all enabled", setup: func(f LogFilter) { f.Set("api", true) }, services: []string{"api", "web", "db"}, expected: map[string]bool{"api": true, "web": true, "db": true}},
		{name: "Toggle empty list", setup: func(f LogFilter) {}, services: []string{}, expected: map[string]bool{}},
	}
