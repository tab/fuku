...
func Test_DefaultKeyMap(t *testing.T) {
	...
	assert.NotEmpty(t, km.ToggleLogSub.Keys())
	...
	assert.Contains(t, km.ToggleLogSub.Keys(), " ")
...
func Test_KeyMap_ShortHelp(t *testing.T) {
	...
	assert.Equal(t, km.ToggleLogSub, bindings[2])
	assert.Equal(t, km.Up, bindings[3])
	assert.Equal(t, km.Down, bindings[4])
	assert.Equal(t, km.ToggleLogs, bindings[5])
	assert.Equal(t, km.Autoscroll, bindings[6])
...
func Test_KeyMap_FullHelp(t *testing.T) {
	...
	assert.Equal(t, km.ToggleLogSub, bindings[0][2])
	assert.Equal(t, km.Up, bindings[0][3])
	assert.Equal(t, km.Down, bindings[0][4])
	assert.Equal(t, km.ToggleLogs, bindings[0][5])
	assert.Equal(t, km.Autoscroll, bindings[0][6])
	assert.Equal(t, km.Quit, bindings[0][7])
	assert.Equal(t, km.ForceQuit, bindings[0][8])
}
